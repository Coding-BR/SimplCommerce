...
RUN ARCH=$(uname -m) && if [ "$ARCH" = "x86_64" ]; then curl -SL "https://github.com/rdvojmoc/DinkToPdf/raw/v1.0.8/v0.12.4/64%20bit/libwkhtmltox.so" --output /app/src/SimplCommerce.WebHost/out/libwkhtmltox.so; fi
